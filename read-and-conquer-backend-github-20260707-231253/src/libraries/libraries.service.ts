...
export class LibrariesService {
    ...
    return candidates
      .map((library) => {
        ...
        return {
          ...
          distance,
          occupiedFaction: library.territory?.faction?.factionName ?? null,
        };
      })
  ...
  async findDetail(libraryId: number) {
    ...
    const logs = await this.prisma.territoryLog.groupBy({
      by: ['factionId'],
      where: { libraryId },
      _sum: { scoreDelta: true },
    });
    const factionIds = logs.map((log) => log.factionId);
    const factions = await this.prisma.faction.findMany({
      where: { factionId: { in: factionIds } },
    });
    const factionNameById = new Map(
      factions.map((faction) => [faction.factionId, faction.factionName]),
    );

    return {
      libraryName: library.libraryName,
      occupiedFaction: library.territory?.faction?.factionName ?? null,
      influences: logs.map((log) => ({
        faction: factionNameById.get(log.factionId) ?? null,
        score: log._sum.scoreDelta ?? 0,
      })),
    };
  }
}
